	...
	{                                                                                                    \
		...
		{                                                                                                \
			*out = ((((0xFFFF - inaccum) * in[0] + inaccum * in[2]) >> (16 - outlshift + outrshift)) +   \
					(((0xFFFF - inaccum) * in[1] + inaccum * in[3]) >> (16 - outlshift + outrshift))) >> \
				   1;                                                                                    \
			inaccum += infrac;                                                                           \
			in += (inaccum >> 16) * 2;                                                                   \
			inaccum &= 0xFFFF;                                                                           \
...
static qboolean QDECL S_LoadWavSound(sfx_t *s, qbyte *data, size_t datalen, int sndspeed, qboolean forcedecode)
{
	...
	else
	{
		...
		case 3 /*WAVE_FORMAT_IEEE_FLOAT*/:
			Con_Printf("%s has an unsupported width (%i bits).\n", s->name, info.bitwidth);
			break;
		case 6 /*WAVE_FORMAT_ALAW*/:
			Con_Printf("%s uses unsupported a-law format.\n", s->name);
			break;
		case 7 /*WAVE_FORMAT_MULAW*/:
			Con_Printf("%s uses unsupported mu-law format.\n", s->name);
			break;
		case 0xfffe /*WAVE_FORMAT_EXTENSIBLE*/:
		default:
			Con_Printf("%s has an unsupported format (%#" PRIX16 ").\n", s->name, info.format);
			break;
		}
		return false;
	}
