...
static pint64_t QCC_Eval_Int(const QCC_eval_t *eval, QCC_type_t *type)
{
	...
	case ev_uint:
		return eval->_uint;
	case ev_boolean:
		return QCC_Eval_Int(eval, type->parentclass); // bools are weird.
	case ev_int64:
		return eval->i64;
	case ev_uint64:
		return eval->u64;
...
static float QCC_Eval_Float(const QCC_eval_t *eval, QCC_type_t *type)
{
	...
	case ev_uint:
		return eval->_uint;
	case ev_boolean:
		return QCC_Eval_Float(eval, type->parentclass); // bools are weird.
	case ev_int64:
		return eval->i64;
	case ev_uint64:
		return eval->u64;
...
#define QCC_PR_EmulationFunc(n) QCC_PR_GetSRef(NULL, #n, pr_scope, false, 0, 0)
		...
		case OP_DIV_U:
			...
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_DIV_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		...
		case OP_ADD_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_ADD_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_SUB_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_SUB_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_MUL_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_MUL_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c; // carry/overflow differ, but we don't provide such feedback.
		...
		case OP_BITAND_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITAND_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_BITOR_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITOR_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_BITXOR_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITXOR_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_BITNOT_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITNOT_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_BITCLR_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITCLR_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_LSHIFT_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_LSHIFT_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_GE_U:
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LE_U], var_b, var_a, NULL, flags); // just swap the args, fewer opcodes needed.
		case OP_GT_U:
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LT_U], var_b, var_a, NULL, flags); // just swap the args, fewer opcodes needed.
		case OP_EQ_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_EQ_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;
		case OP_NE_U:
			var_a.cast = type_integer;
			var_b.cast = type_integer;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_NE_I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint;
			return var_c;

		case OP_ADD_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_ADD_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_SUB_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_SUB_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_MUL_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_MUL_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		...
		case OP_BITAND_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITAND_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_BITOR_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITOR_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_BITXOR_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITXOR_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_BITNOT_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITNOT_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_BITCLR_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_BITCLR_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_LSHIFT_U64I:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_LSHIFT_I64I], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_GE_U64:
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LE_U64], var_b, var_a, NULL, flags);
		case OP_GT_U64:
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LT_U64], var_b, var_a, NULL, flags);
		case OP_EQ_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_EQ_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_NE_U64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			var_c = QCC_PR_StatementFlags(&pr_opcodes[OP_NE_I64], var_a, var_b, NULL, flags);
			var_c.cast = type_uint64;
			return var_c;
		case OP_GE_I64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LE_I64], var_b, var_a, NULL, flags);
		case OP_GT_I64:
			var_a.cast = type_int64;
			var_b.cast = type_int64;
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LT_I64], var_b, var_a, NULL, flags);
		case OP_GE_D:
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LE_D], var_b, var_a, NULL, flags);
		case OP_GT_D:
			return QCC_PR_StatementFlags(&pr_opcodes[OP_LT_D], var_b, var_a, NULL, flags);
...
static QCC_sref_t QCC_TryEvaluateCast(QCC_sref_t src, QCC_type_t *cast, pbool implicit)
{
	...
	else if (totype == ev_variant || src.cast->type == ev_variant || (totype == ev_field && totype == src.cast->type && (tmp->aux_type->type == ev_variant || src.cast->aux_type->type == ev_variant)))
	{
		src.cast = cast;
	...
	/*these casts are acceptable but probably an error (so warn when implicit)*/
		...
		|| (totype == ev_entity && src.cast->type == ev_integer) || (totype == ev_entity && src.cast->type == ev_float && flag_qccx) || (totype == ev_integer && src.cast->type == ev_entity) || (totype == ev_float && src.cast->type == ev_entity && flag_qccx) || (totype == ev_string && src.cast->type == ev_float && flag_qccx) || (totype == ev_float && src.cast->type == ev_string && flag_qccx))
	{
		// direct cast
		if (implicit && typecmp_lax(src.cast, cast))
		{
...
{
	QCC_sref_t r;
	if ((cast->type == ev_accessor && cast->parentclass == src.cast) || (src.cast->type == ev_accessor && src.cast->parentclass == cast))
	{
		if (implicit)
		{
			char typea[256];
...
void QCC_PR_FinaliseFunctions(void)
{
	...
	if (numinitstatements)
	{
		...
		if (f == functions + numfunctions)
			for (f = functions + 1; f < functions + numfunctions; f++)
				if (!strcmp(f->name, "_atinit"))
					break;
		if (f == functions + numfunctions)
			for (f = functions + 1; f < functions + numfunctions; f++)
		...
		if (f == functions + numfunctions)
			for (f = functions + 1; f < functions + numfunctions; f++)
				if (!strcmp(f->name, "initents"))
					break;
		if (f == functions + numfunctions)
			for (f = functions + 1; f < functions + numfunctions; f++)
				if (!strcmp(f->name, "m_init") || !strcmp(f->name, "CSQC_Init") || !strcmp(f->name, "worldspawn"))
					break;
		if (f == functions + numfunctions)
...
#define KEYWORD(x)                                                                                                                                            \
	if (!STRCMP(name, #x) && keyword_##x)                                                                                                                     \
		do                                                                                                                                                    \
		{                                                                                                                                                     \
			if (keyword_##x)                                                                                                                                  \
				QCC_PR_ParseWarning(WARN_KEYWORDDISABLED, "\"" #x "\" keyword used as variable name%s", keywords_coexist ? " - coexisting" : " - disabling"); \
	...
	for (; ins < insmax; ins++)
	{
		...
		while (def)
		{
			...
			if (type && typecmp(def->type, type))
			{
				...
				{
					if (!pr_scope && (!strcmp("droptofloor", def->name) ||	// vanilla
...
void QCC_PR_ParseDefs(const char *classname, pbool fatal_unused)
{
	...
	if (flag_acc)
	{
		...
		if (QCC_PR_CheckKeyword(keyword_objdata, "objdata")) // reacc support.
		{
			...
			else if (!QCC_PR_GetDef(type_void, "end_sys_globals", NULL, false, 0, false))
	...
	do
	{
		...
		{
			QCC_type_t *old;
			if (externfnc || shared || isconstant || isvar || forceused || dostrip || allowinline || dowrap || doweak || accumulate || aliasof || deprecated || (isstatic && !defaultstatic) || (noref && !defaultnoref) || (nosave && !defaultnosave))
		...
		else if (isauto)
		{
			...
			else
			{
				...
				if (!f.cast)
					f = QCC_PR_EmulationFunc(malloc);
				if (!f.cast)
					QCC_PR_ParseError(ERR_BADEXTENSION, "memalloc not yet defined, cannot define __auto %s at global scope", name);
				if (QCC_OPCodeValid(&pr_opcodes[OP_CALL1H]))
